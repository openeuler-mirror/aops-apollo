...
class TestCveScanCallback(unittest.TestCase):
    ...
    def test_cve_scan_callback_should_return_400_when_request_without_args(self):
        response = client.post(VUL_TASK_CVE_SCAN_CALLBACK, headers=header_with_token)
        self.assertEqual(400, response.status_code)

    def test_cve_scan_callback_should_return_405_when_request_with_incorrect_method(self):
        response = client.put(VUL_TASK_CVE_SCAN_CALLBACK, headers=header_with_token)
        self.assertEqual(405, response.status_code)

    def test_cve_scan_callback_should_return_param_error_when_request_with_incorrect_args(self):
        response = client.post(VUL_TASK_CVE_SCAN_CALLBACK, data=json.dumps({}), headers=header_with_token)
