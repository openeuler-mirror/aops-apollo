...
class CveFixManagerTestCase(unittest.TestCase):
    ...
    def test_create_task_should_return_SUCCEED_directly_when_task_is_in_cache(
            self):
        fake_task = Mock()
    ...
    @mock.patch.object(TaskProxy, 'get_cve_basic_info')
    def test_create_task_should_return_SUCCEED_and_query_from_db_when_task_in_db(
            self, mock_proxy):
        proxy = TaskProxy(configuration)
        manager = CveFixManager(proxy, "id1")
        fake_task = {"mock": Mock()}
    ...
    def test_pre_handle_should_return_False_when_init_task_fail(
            self, mock_init_status):
        proxy = TaskProxy(configuration)
    ...
    @mock.patch.object(TaskProxy, 'init_cve_task')
    def test_pre_handle_should_return_True_when_init_task_succeed(
            self, mock_init_status, mock_update_time):
        proxy = TaskProxy(configuration)
        manager = CveFixManager(proxy, Mock())
    ...
    def test_post_handle_should_be_correct(
    ...
    def test_fault_handle_should_have_correct_step(
            self, mock_set_cve_progress, mock_fix_task_status):
        proxy = TaskProxy(configuration)
        fake_task_id = Mock()
