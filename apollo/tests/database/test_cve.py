...
class TestCveMysqlProxy(DatabaseTestCase):
    ...
    def test_get_cve_task_host(self):
        ...
        data = {
            "cve_list": ["qwfqwff5", "qwfqwff6", "not_exist_id"],
            "username": "admin",
        }
        expected_query_result = {"result": {"qwfqwff5": [{"host_id": 2, "host_name": "host2", "host_ip": "127.0.0.2"}]}}
        self.assertEqual(
            self.cve_database.get_cve_task_hosts(data),
...
class TestCveProxy(DatabaseTestCase):
    ...
    def test_save_unaffected_cve(self):
        ...
        self.assertEqual(
            self.cve_database.save_unaffected_cve(filename, cve_rows, cve_pkg_rows, cve_pkg_docs),
            SUCCEED,
        )
