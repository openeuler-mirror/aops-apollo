...
class TestRepoDatabase(unittest.TestCase):
    ...
    def test_import_repo(self):
        ...
        repo_count = self.repo_database.session.query(func.count(Repo.repo_id))\
            .filter(Repo.username == data["username"]).scalar()
        self.assertEqual(self.repo_database.import_repo(data), 1105)

        new_repo_count = self.repo_database.session.query(func.count(Repo.repo_id))\
            .filter(Repo.username == data["username"]).scalar()
    ...
    def test_query_repo(self):
        ...
        data = {
            "username": "admin",
            "repo_name_list": ["repo1", "repo_not_exist"]
        }
